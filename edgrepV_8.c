...
int  names[26], anymarks, nbra, subnewa, subolda, fchange, wrapp, bpagesize = 20, bufp = 0;
...
char  line[70];  char* linp = line; char buf[BUFSIZE];
...
void readfile(const char* c) {
    ...
    filename(c);  init();
    addr2 = zero;  if ((io = open((const char*)file, 0)) < 0) {
        lastc = '\n';
        error(file);
    }
